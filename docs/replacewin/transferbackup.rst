...
To start things off, go into the :menuselection:`Applications Menu (the bottom-left bird icon) --> System --> Transfer Tool` to launch Transfer Tool.
...
On the next page in Transfer Tool make sure the switch at the top saying :guilabel:`Backup data from an external source` is switched on. After that, click on the dropdown menu on the right of that switch and select your Windows partition.
...
After doing that you can proceed to install Feren OS onto your computer.

Next Steps
----------------

* `Installing Feren OS <https://feren-os-user-guide.readthedocs.io/en/latest/replacewin/install.html>`_